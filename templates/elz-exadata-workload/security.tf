...
locals {
  osms_dynamic_group = {
    dynamic_group_name        = "DG-${var.environment_prefix}-${var.workload_name}"
  ...
  bastion = {
    name = "BAS-${var.environment_prefix}-${var.workload_name}"
  }
...
module "bastion" {
  ...
  target_subnet_id                     = module.exadata_workload_expansion_spoke.subnets["${var.workload_prefix}-EXA-SPK-SUB-${local.region_key[0]}-LB"]
