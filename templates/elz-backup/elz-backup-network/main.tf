...
locals {
  vcn_hub = {
    name      = "VCN-${var.environment_prefix}-HUB-BACKUP"
    dns_label = "hubbcklabel"
  }
  vcn-hub-info = {
    hub_public_subnet_display_name  = "SUB-${var.environment_prefix}-HUB-${local.region_key[0]}001"
    hub_public_subnet_description   = "Hub Public Subnet Backup"
    hub_public_subnet_dns_label     = "publabelbck"
    hub_private_subnet_display_name = "SUB-${var.environment_prefix}-HUB-${local.region_key[0]}002"
